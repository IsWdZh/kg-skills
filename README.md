...
> 本技能包经「公积金场景」62 份文档（省直/兰州/嘉峪关三地）全流程试跑验证：3 个单元场景 × 3 地区版本 × 轻/标准/推理三档治理，9 个知识包完成内部试跑发布，9 类服务问题全部通过验收。试跑通过不等同于生产终审。
...
| `kg-produce` | 四形态生产：知识块/问答对/Wiki/图谱自由搭配 | 50-知识资产/ |
| `kg-coordinate` | 多形态协同：锚点校验、互链、一致性 | 协同校验报告 |
| `kg-package` | 知识包组装发布：审核、不可变快照、哈希、版本回滚、数据库投影 | 60-知识包/、80-知识库导出/ |
